 ...
 * --
 * @(#) $Id: irc_client.c,v 1.1 2000/05/13 02:14:12 keybuk Exp $
 *
...
int ircclient_welcome(struct ircproxy *p) {
  ...
  if (p->channels) {
    ...
    while (c) {
      ...
      irclog_recall(p, &(c->log), TODO_CFG_INITRECALL);

      c = c->next;
    }
  }

  irclog_recall(p, &(p->misclog), TODO_CFG_INITRECALL);
